/**
 * Last applications for course
 * 
 * Backend SQL Query:
 * SELECT * FROM szkolenie_kursant sk 
 * INNER JOIN szkolenie s ON s.szkolenie_id = sk.szkolenie_id 
 * INNER JOIN termin t ON s.termin_id = t.termin_id 
 * WHERE t.termin_id = {id} 
 * ORDER BY sk.dodano DESC 
 * LIMIT 10;
 */
export default async function handler(req, res) {
	...
	try {
		...
		const response = await fetch(`${apiUrl}/api/course/lastapplictions/${id}`);
		...
		console.error('Last applications error:', error);
